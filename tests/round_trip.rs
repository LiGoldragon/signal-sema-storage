...
use signal_sema_storage::{
    ...
    DocumentKey, DocumentKind, DocumentPayload, EncodedSchemaArchive, FixtureScope, FrameMessage,
    IdentityAssignment, IdentityIntent, MintedUniverse, NameTableBytes, Rejection, Reply, Request,
    SchemaWholeHandle, SlotIdentifier, TypeIdentity, Wire,
};
#[test]
...
fn encoded_schema_and_logos_document_payloads_round_trip() {
    ...
    let archived_schema = EncodedSchemaArchive::from_schema(&schema).expect("archive schema");
    assert_eq!(archived_schema.to_schema().expect("load schema"), schema);
    let payloads = [
        (
